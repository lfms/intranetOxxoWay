...
// ********************************************************************************
public static String Encriptar ( String strPlainText ) {
    String encriptacion1 = "";
    String encriptacion2 = "";

    if(strPlainText.trim().length()>16){
        encriptacion1=Encriptar16(strPlainText.substring(0,16));
        encriptacion2=Encriptar16(strPlainText.substring(16));
    }else{
        encriptacion1=Encriptar16(strPlainText.trim());
    }

    return encriptacion1+encriptacion2;
}

public static String Encriptar16 ( String strPlainText ) {
	String strResultado = "";
    if(strPlainText.length()<=16){
        strPlainText=fillRight(strPlainText, 16-strPlainText.length(), "*");
    }
	...
	try {
		...
		byte[] auxi = new byte [ BLOCK_SIZE ];
		int iIdx;
		
		for ( iIdx = 0; iIdx < keysize; iIdx++ )
			kb [ iIdx ] = ( byte ) iIdx;
		
		int iNum;
		for ( iIdx = 0; iIdx < BLOCK_SIZE; iIdx++ ) {
			iNum        = ( int  ) strPlainText.charAt ( iIdx );
			pt [ iIdx ] = ( byte ) iNum;
		}
		
		Object Key = makeKey ( kb, BLOCK_SIZE );
		ct = blockEncrypt ( pt, 0, Key, BLOCK_SIZE );
		strTemp = toString ( ct );
		auxi = ct;

		byte[] cpt = blockDecrypt ( ct, 0, Key, BLOCK_SIZE );
		strResultado = toString ( ct );
	}
	catch ( Exception x ) {
		strResultado = "";
		return strResultado;
...
public static String Desencriptar ( String strCipherText ) {
    String desencriptacion1 = "";
    String desencriptacion2 = "";

    if(strCipherText.trim().length()>32){
        desencriptacion1=Desencriptar16(strCipherText.substring(0,32));
        desencriptacion2=Desencriptar16(strCipherText.substring(32));
    }else{
        desencriptacion1=Desencriptar16(strCipherText.trim());
    }

    return desencriptacion1+desencriptacion2;
}

// ********************************************************************************
public static String Desencriptar16 ( String strCipherText ) {
	String strResultado = "";
	...
	try {
		...
		iAux = 0;
		do {
			strTemp = strCipherText.substring ( iIdx, iIdx + 2 );
			sTempo = Short.parseShort ( strTemp, 16 );
			ct [ iAux ] = ( byte ) sTempo;
			iAux++;
			iIdx += 2;
		} while ( iIdx < BLOCK_SIZE * 2 );

		//System.out.println ( "ct             = " + toString ( ct ) );
		Object key   = makeKey ( kb, BLOCK_SIZE );
		cpt = blockDecrypt ( ct, 0, key, BLOCK_SIZE );

		String strTempo  = "";
		strTempo     = toString(cpt);
		strResultado = strTempo.substring(1,2);
		strResultado = strResultado + strTempo.substring(3,4);
		strResultado = strResultado + strTempo.substring(5,6);
		strResultado = strResultado + strTempo.substring(7,8);
		strResultado = strResultado + strTempo.substring(9,10);
		strResultado = strResultado + strTempo.substring(11,12);
		strResultado = strResultado + strTempo.substring(13,14);
		strResultado = strResultado + strTempo.substring(15,16);
		strResultado = strResultado + strTempo.substring(17,18);
		strResultado = strResultado + strTempo.substring(19,20);
		strResultado = strResultado + strTempo.substring(21,22);
		strResultado = strResultado + strTempo.substring(23,24);
		strResultado = strResultado + strTempo.substring(25,26);
		strResultado = strResultado + strTempo.substring(27,28);
		strResultado = strResultado + strTempo.substring(29,30);
		strResultado = strResultado + strTempo.substring(31,32);
	}
	catch ( Exception x ) {
		strResultado = "";
		return strResultado;
	...
	return strResultado.replaceAll("A", "");
}

public static String fillRight(String valor, int tot, String car) {
        String resultado = "";
        for (int i = 0; i < tot; i++)
            resultado += car;
...
//...........................................................................
    ...
    public static void main ( String[] args ) {
    	String strNumTarjetaN = "";
    	String strNumTarjetaE = "";

	//----- Proceso de Encripcion
//    strNumTarjetaN = "1111222";
//    strNumTarjetaN = "1111222233";
//    strNumTarjetaN = "111122223333";
//    strNumTarjetaN = "11112222333344";
//    strNumTarjetaN = "1111222233334444";
//    strNumTarjetaN = "111122223333444455";
    strNumTarjetaN = "525678XP67577091";

    strNumTarjetaE = "";
	strNumTarjetaE = Encriptar ( strNumTarjetaN );
	System.out.println ( "---Encripcion" );
	System.out.println ( "strNumTarjetaN = " + strNumTarjetaN );
	System.out.println ( "strNumTarjetaE = " + strNumTarjetaE );
	System.out.println ( "" );

	//----- Proceso de Desencripcion
	strNumTarjetaN = "";
//	strNumTarjetaE = "DF7EE508753511E3759A6E0D89CE49C9";
//	strNumTarjetaN = Desencriptar ( strNumTarjetaE );
        strNumTarjetaE = "C4F54B030909693630C63D860C54A591";
	strNumTarjetaN = Desencriptar ( strNumTarjetaE );
